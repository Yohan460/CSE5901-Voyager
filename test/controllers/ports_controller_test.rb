...
class PortsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create port" do
    assert_difference('Port.count') do
      post ports_url, params: { port: { country: @port.country, description: @port.description, location: @port.location } }
    end
  ...
  test "should update port" do
    patch port_url(@port), params: { port: { country: @port.country, description: @port.description, location: @port.location } }
